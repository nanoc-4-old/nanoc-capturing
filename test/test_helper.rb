  ...
  def test_dependencies
    in_site do
      ...
      File.open('lib/helpers.rb', 'w') do |io|
        io.write 'include Nanoc::Capturing::Helper'
      end
      ...
      # Compile once
      File.write(
        'content/includee.erb',
        '{<% content_for :blah do %>Old content<% end %>}')
      Nanoc::Compiler.new(mock_site).run
      assert_equal '[Old content]', File.read('output/includer.html')

      # Compile again
      File.write(
        'content/includee.erb',
        '{<% content_for :blah do %>New content<% end %>}')
      Nanoc::Compiler.new(mock_site).run
